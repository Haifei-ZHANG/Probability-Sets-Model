...
#        pool_length = int((len(y)*3)/4) # 5 % train, 75 % pool, 20 % test
        ...
        CL_accuracy = np.zeros(budget+2)  
        QB_accuracy = np.zeros(budget+2)
        CL_classes = np.zeros(budget+2)
        QB_classes = np.zeros(budget+2)       
        CL_uncertainty_scores = np.zeros(budget)  
...
#        EC_uncertainty_scores = np.zeros(budget)
        ...
        CL_used_budget = np.zeros(n_thresholds)  
...
#                X_train, y_train,  X_test, y_test = X[train_index],  y[train_index], X[test_index], y[test_index]
                ...
                CL_accuracy[budget+1] += final_accuracy  
                CL_classes[budget+1] += 1  
...
#                EC_accuracy[budget+1] += final_score                                 
    ...
    #                y_train =  y[train_index]
                ...
                CL_accuracy[0] += clf.score(X_test, y_test)   
                CL_classes[0] += len(list(clf.classes_))/n_class_total   
                ...
                for iteration in range(budget):
                    ...
                    CL_accuracy[iteration+1] += current_accuracy
                    CL_classes[iteration+1] += len(list(clf.classes_))/n_class_total 
                    current_score= np.mean([confidences[i] for i in instance_order_cl[:batch_size]])                        
                    CL_uncertainty_scores[iteration] += current_score
                    if iteration >= check_stop_place:
                        for th in range(n_thresholds):
                            if current_score > thresholds[th] and CL_filled_check_point[th] == 0:
                                CL_filled_check_point[th] = 1
                                CL_gained_accuracy[th] += current_accuracy
                                CL_used_budget[th] += (iteration+1)/budget                                               
                for th in range(n_thresholds):
                    if CL_filled_check_point[th] == 0:
                        CL_gained_accuracy[th] += final_accuracy
                        CL_used_budget[th] += 1   
                end = timer()
                print("CL sampling (in seconds): ", end - start)                    
...
#        print(int(stop_place/(folds*n_repetition)))
        ...
        plt.plot(CL_accuracy/(folds*n_repetition), linewidth=0.5, color='tab:red', label='CL approach')
...
#        plt.plot(EC_accuracy/(folds*n_repetition), linewidth=0.5, color='tab:green', label='EC approach')
        ...
        plt.savefig('results/active learning/{}_CL_noisy_label_accuracy-vs_n_queries.png'.format(data_names[d]), bbox_inches='tight', dpi=600)
        plt.show()
        plt.close()
        
        fig = plt.figure(figsize=(6,4))
        
        plt.plot(CL_classes/(folds*n_repetition), linewidth=0.5, color='tab:red', label='CL approach')
...
#        plt.plot(EC_accuracy/(folds*n_repetition), linewidth=0.5, color='tab:green', label='EC approach')
        ...
        plt.savefig('results/active learning/{}_CL_noisy_label_known_classes-vs_n_queries.png'.format(data_names[d]), bbox_inches='tight', dpi=600)
        plt.show()
        plt.close()
        
        fig = plt.figure(figsize=(6,4))
        
        plt.plot(CL_uncertainty_scores/(folds*n_repetition), linewidth=0.5, color='tab:red', label='CL approach')
...
#        plt.plot(EC_uncertainty_scores/(folds*n_repetition), linewidth=0.5, color='tab:green', label='EC approach')
        ...
        plt.savefig('results/active learning/{}_CL_noisy_label_scores-vs_n_queries.png'.format(data_names[d]), bbox_inches='tight', dpi=600)
        plt.show()
        plt.close()
...
#        plt.plot(EC_uncertainty_scores/(folds*n_repetition), linewidth=0.5, color='tab:green', label='EC approach')
        ...
        plt.savefig('results/active learning/{}_CL_noisy_label_gained-accuracy-vs_threshold.png'.format(data_names[d]), bbox_inches='tight', dpi=600)
        plt.show()
        plt.close()
        
        fig = plt.figure(figsize=(6,4))
        
        plt.plot(CL_used_budget/(folds*n_repetition), linewidth=0.5, color='tab:red', label='CL approach')
...
#        plt.plot(EC_uncertainty_scores/(folds*n_repetition), linewidth=0.5, color='tab:green', label='EC approach')
        ...
        plt.savefig('results/active learning/{}_CL_noisy_label_used_budget-vs_threshold.png'.format(data_names[d]), bbox_inches='tight', dpi=600)
        plt.show()
        plt.close()