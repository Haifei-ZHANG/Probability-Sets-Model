...
#            for train_index, test_index in Kf.split(y):   
        ...
        CL_accuracy_acceptance_rate = np.zeros(min_test_lengths)              
        QB_accuracy_acceptance_rate = np.zeros(min_test_lengths)
        CL_score_acceptance_rate = np.zeros(min_test_lengths)              
        QB_score_acceptance_rate = np.zeros(min_test_lengths)
        CL_accuracy_threshold = np.zeros(min_test_lengths)              
        QB_accuracy_threshold = np.zeros(min_test_lengths)
        CL_acceptance_rate_threshold = np.zeros(min_test_lengths)              
        QB_acceptance_rate_threshold = np.zeros(min_test_lengths)        
        CL_acceptance_rate = np.zeros(n_thresholds)  
        QB_acceptance_rate = np.zeros(n_thresholds)     
        CL_gained_accuracy = np.zeros(n_thresholds)  
...
#                y_train =  y[train_index]
                ...
                CL_filled_check_point = np.zeros(n_thresholds)  
                ...
                confidences = np.zeros(len(y_test))
                for i in range(len(y_test)):
                    confidences[i] = predicted_probabilities[i, max_p_index[i]] 
                instance_order_cl = list(np.argsort(confidences))
                for i in range(len(y_test)):
                    current_accuracy = np.mean(correctness[instance_order_cl[i:]])
                    CL_accuracy_acceptance_rate[i] += current_accuracy
                    current_score = confidences[instance_order_cl[i]]  
                    CL_score_acceptance_rate[i] += current_score             
                    for th in range(n_thresholds):
                        if  confidences[instance_order_cl[i]] > thresholds[th] and CL_filled_check_point[th] == 0:
                            CL_filled_check_point[th] = 1
                            CL_gained_accuracy[th] += current_accuracy
                            CL_acceptance_rate[th] += len(instance_order_cl[i:])/n_instance  
                for i in range(len(y_test), min_test_lengths):
                    CL_accuracy_acceptance_rate[i] += current_accuracy
                    CL_score_acceptance_rate[i] += current_score             
                for th in range(n_thresholds):
                    if CL_filled_check_point[th] == 0:
                        CL_gained_accuracy[th] += 1
                        CL_acceptance_rate[th] += 0 
                ...
                for i in range(len(y_test)):
                    ...
                    smallest_alphas[i] = np.sum([confidences[i]] + [sorted_probability_set[i,max_p_index_ensemble_members[i]] for i in range(ensemble_size)])/(ensemble_size +1)
                    for position in range(ensemble_size):
                        if predicted_classes[i] != class_list[max_p_index_ensemble_members[position]]:
                           smallest_alphas[i] = np.sum([confidences[i]] + [sorted_probability_set[i,max_p_index_ensemble_members[i]] for i in range(position)])/(ensemble_size +1)
                           break
        ...
        plt.plot(CL_accuracy_acceptance_rate/(folds*n_repetition), linewidth=0.5, color='tab:red', label='CL approach')
...
#        plt.plot(EC_accuracy/(folds*n_repetition), linewidth=0.5, color='tab:green', label='EC approach')
        ...
        plt.savefig('results/score-rejection-curves/{}_CL_noisy_label_accuracy-rejection.png'.format(data_names[d]), bbox_inches='tight', dpi=600)
#        plt.savefig('results/[10 x 10 cross-validation with train = 9 folds] RF accuracy-rejection curve/{}_accuracy-rejection.png'.format(data_names[d]), bbox_inches='tight', dpi=600)
        ...
        plt.plot(CL_score_acceptance_rate/(folds*n_repetition), linewidth=0.5, color='tab:red', label='CL approach')
...
#        plt.plot(EC_accuracy/(folds*n_repetition), linewidth=0.5, color='tab:green', label='EC approach')
        ...
        plt.savefig('results/score-rejection-curves/{}_CL_noisy_label_score-rejection.png'.format(data_names[d]), bbox_inches='tight', dpi=600)
#        plt.savefig('results/[10 x 10 cross-validation with train = 9 folds] RF accuracy-rejection curve/{}_accuracy-rejection.png'.format(data_names[d]), bbox_inches='tight', dpi=600)
        ...
        plt.plot(CL_gained_accuracy/(folds*n_repetition), linewidth=0.5, color='tab:red', label='CL approach')
...
#        plt.plot(EC_accuracy/(folds*n_repetition), linewidth=0.5, color='tab:green', label='EC approach')
        ...
        plt.savefig('results/score-rejection-curves/{}_CL_noisy_label_gained_accuracy-threshold.png'.format(data_names[d]), bbox_inches='tight', dpi=600)
#        plt.savefig('results/[10 x 10 cross-validation with train = 9 folds] RF accuracy-rejection curve/{}_accuracy-rejection.png'.format(data_names[d]), bbox_inches='tight', dpi=600)
        ...
        plt.plot(CL_acceptance_rate/(folds*n_repetition), linewidth=0.5, color='tab:red', label='CL approach')
...
#        plt.plot(EC_accuracy/(folds*n_repetition), linewidth=0.5, color='tab:green', label='EC approach')
        ...
        plt.savefig('results/score-rejection-curves/{}_CL_noisy_label_acceptance_rate-threshold.png'.format(data_names[d]), bbox_inches='tight', dpi=600)
#        plt.savefig('results/[10 x 10 cross-validation with train = 9 folds] RF accuracy-rejection curve/{}_accuracy-rejection.png'.format(data_names[d]), bbox_inches='tight', dpi=600)
...
#         plt.savefig('results/score-rejection-curves/{}_CL_accuracy-rejection.png'.format(data_names[d]), bbox_inches='tight', dpi=600)
# #        plt.savefig('results/[10 x 10 cross-validation with train = 9 folds] RF accuracy-rejection curve/{}_accuracy-rejection.png'.format(data_names[d]), bbox_inches='tight', dpi=600)
...
#         plt.savefig('results/score-rejection-curves/{}_CL_score-rejection.png'.format(data_names[d]), bbox_inches='tight', dpi=600)
# #        plt.savefig('results/[10 x 10 cross-validation with train = 9 folds] RF accuracy-rejection curve/{}_accuracy-rejection.png'.format(data_names[d]), bbox_inches='tight', dpi=600)
...
#         plt.savefig('results/score-rejection-curves/{}_CL_gained_accuracy-threshold.png'.format(data_names[d]), bbox_inches='tight', dpi=600)
# #        plt.savefig('results/[10 x 10 cross-validation with train = 9 folds] RF accuracy-rejection curve/{}_accuracy-rejection.png'.format(data_names[d]), bbox_inches='tight', dpi=600)
...
#         plt.savefig('results/score-rejection-curves/{}_CL_acceptance_rate-threshold.png'.format(data_names[d]), bbox_inches='tight', dpi=600)
# #        plt.savefig('results/[10 x 10 cross-validation with train = 9 folds] RF accuracy-rejection curve/{}_accuracy-rejection.png'.format(data_names[d]), bbox_inches='tight', dpi=600)
